...
export default {
  ...
  methods: {
    ...
    changeTab () {
      ...
      // article-list组件需要 拿到传过去的页签  看看是否是自己所在的页签
      // 如果是自己所在的页签  就需要判断一下  自己的组件是否有滚动 如果有滚动数据 就滚动对应的位置
    ...
    async addChannel (channel) {
      await addChannel(channel) // 完成写入本地缓存的操作
      this.channels.push(channel) // 自身加一个频道影响子组件
    }
  },
  created () {
    this.getMyChannels()
